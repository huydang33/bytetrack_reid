...
class Trainer:
    def __init__(self, exp, args):
        ...
        self.isEarlyStopping = self.args.early_stopping
    ...
    def train_in_epoch(self):
        for self.epoch in range(self.start_epoch, self.max_epoch):
            ...
            if self.loss < self.best_val_loss:
                self.best_val_loss = self.loss
                self.counter = 0
            else:
                self.counter += 1
                if self.counter >= self.patience and self.isEarlyStopping:
    ...
    def before_train(self):
        ...
        feature_extractor_model = torch.nn.Sequential(*list(feature_extractor_model.children())[:-1])
        feature_extractor_model.to(self.device)
